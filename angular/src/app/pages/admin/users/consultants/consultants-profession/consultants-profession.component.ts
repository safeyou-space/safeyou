...
@Component({
  selector: 'app-consultants-profession',
  templateUrl: './consultants-profession.component.html',
  styleUrls: ['./consultants-profession.component.css'],
  animations: [
    trigger('page', [
      state('open', style({
        opacity: '1',
...
export class ConsultantsProfessionComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.url = `${environment.baseUrl}/${this.country}/${this.language}/${environment.admin.consultants.getProfessionList}`;
  ...
  editProfession (data) {
    this.requestService.updateData(this.url, data, this.itemId).subscribe((items) => {
      this.getProfessionList(this.urlConstructor());
      ConsultantsComponent.instance.getData(`${environment.baseUrl}/${this.country}/${this.language}/${environment.admin.consultants.get}`);
      this.formProfession.reset();
      this.addProfession = false;
    })
  }
