...
export class PrivacyPolicyComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.getData(this.url + '?title=privacy_policy');
  }

  getData(url) {
    forkJoin([this.requestService.getData(`${this.url}?title=privacy_policy`), this.requestService.getData(`${this.url}?title=privacy_policy-18`)]).subscribe((result: any) => {
    ...
    if (this.form.valid) {
      ...
      for (let i in form) {
        if (i == 'content') {
          ...
          newData[i] = form[i];
          newContent['title'] = 'privacy_policy-18';
        }
      }
