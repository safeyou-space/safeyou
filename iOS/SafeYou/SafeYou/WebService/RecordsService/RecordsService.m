...
{
    [self.networkManager GET:[self endpointRecords] parameters:nil progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
...
- (void)getAllRecordsWithSearch:(NSString *)searchText complition:(void(^)(NSArray <RecordSearchResult *> *searchResults))complition failure:(void(^)(NSError *error))failure
{
    ...
    [self.networkManager GET:[self endpointRecords] parameters:params progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
...
{
    [self.networkManager GET:[self endpointForRecordId:recordId] parameters:nil progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
...
{
    [self.networkManager DELETE:[self endpointForRecordId:recordId] parameters:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        if (complition) {
...
{
    [self.networkManager POST:[self endpointForSendRecord:recordId] parameters:params progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        if (complition) {
            complition(responseObject);
        }
...
- (void)addRecord:(NSString *)recordFilePath params:(NSDictionary *)recordParams complition:(void(^)(id response))complition failure:(void(^)(NSError *error))failure
{
    ...
    [self.networkManager POST:@"record" parameters:nil constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
